...
def document_upload_new_file():
    ...
    if request.method == 'GET':
        return render_template('./document_replace_file.html', doc=doc)
    ...
    print("Change file for doc BEFORE ASIGN NEW FILE:  file: ", doc.id, uploaded_file.name)
    ...
    db.session.commit()  
    print("Change file for doc AFTER ASIGN NEW FILE:  file: ", doc.id, doc.ufile_id, uploaded_file.name)
...
@doc.route('/document_replace_upload_file2/<int:selected_document_id>', methods=['GET', 'POST'])
def document_replace_upload_file2(selected_document_id):
	print("in document_upload_file2 selected_document_id is: ", selected_document_id)
	doc = document_select2(selected_document_id)
	return document_upload_new_file2(doc.id)	
...
def download_document_file():
    ...
    downloaded_file = Ufile.query.filter(Ufile.id==doc.ufile_id).first() 
...
@doc.route('/download_document_file2/<int:selected_document_id>', methods=['GET', 'POST'])
def download_document_file2(selected_document_id):
    
    doc = document_select2(selected_document_id)
        
    print("IN documents/download_document_file2 doc: ", doc)
    ###import pdb; pdb.set_trace()
