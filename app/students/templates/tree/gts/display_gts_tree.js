        ...
        $(go.Diagram, "fullDiagram",  // each diagram refers to its DIV HTML element by id
			{
				...
				"clickCreatingTool.insertPart": function(loc) {  // scroll to the new node
					...
					if (node !== null) {
						this.myFullDiagram.select(node);
						this.myFullDiagram.commandHandler.scrollToPart(node);
						this.myFullDiagram.commandHandler.editTextBlock(node.findObject("NAMETB"));
					}
					return node;
				},
				/*****************************************************************************************/
			  ...
			  { fromSpot: go.Spot.RightSide, toSpot: go.Spot.LeftSide },   // coming out from right side  going into at left side
				...
				//$(go.Panel, "Horizontal",
					...
					$(go.Panel, "Table",
						...
						$("TreeExpanderButton",  {
							row: 6, column: 0,
							alignment: go.Spot.Bottom, alignmentFocus: go.
							Spot.Top,
							name: 'TREEBUTTON' },
							$(go.TextBlock, " Click to open net level ", { margin: 2 } ) ),	// FROM https://gojs.net/latest/intro/buttons.html		
...
function switchNodeColor() {
	...
	m.commtTransaction("changeNodeColor");
