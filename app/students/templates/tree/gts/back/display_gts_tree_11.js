...
function display_node_by_loop( parent_gt, num_of_nodes )  {  //////////////////// DISPLAY-NODE ////////////////////////////////
	...
	parent_gt_node_struct = {
		...
		id:       parent_gt.id,
		title:    parent_gt.title + ' ' + 'id: ' + String(parent_gt.id),
...
function removeUnchosenBrothers() {
	...
	if ( clicked_nodes.length<1 )  {  // FIRST ONE IS NOT GRAYED
		console.log("CLICKED_ARR IS EMPTY PUSHING 222 ", selected_node.data)
		clicked_nodes.push(selected_node)
	}
		
	else  {
		console.log("GDDDDRRRAAAEEEDDD SELECTED-NODE-DATA GRAYED ", selected_node.data.grayed, selected_node.data.title)
		...
		if (selected_node.data.grayed == "true")  {
			...
			for (let i=0; i<clicked_nodes.length; i++)  {
				...
				if (clicked_nodes[i].data.key == selected_node.data.key)  {
					...
					myFullDiagram.model.setDataProperty(selected_node.data, clicked_nodes[i].data); //Retrieve the original data
	...
	parent_node.findLinksOutOf().each(function(n) {  // ALL CHILDREN OF THE SAME PARENT
		
		console.log("BEFORE IF ", n.data)
		...
		if (n.data.key != selected_node.data.key) {  // GRAY All brothers exceept from selected one itself
			...
			*********************************************************************/
			console.log("AFTER N-DATA ", n.data)
