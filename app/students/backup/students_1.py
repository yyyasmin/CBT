...
def show_student_tree():
    ...
    situations = []
    situation_new_nodes = []
    tmp_situations = Situation.query.filter(Situation.hide == False).filter(Situation.used == True).all()
    for s in tmp_situations:
        situations.append( set_gt_node(s, 0, "false") )  # DUMMY tmp_situations should be students
        situation_new_nodes.append( set_new_emty_gt_node(s, "true") )  # Prepare new empty node in case a user wants to add newe gts of his own
    ...
    thoughts = []
    thoughts_new_nodes = []
    tmp_thoughts = Thought.query.filter(Thought.hide == False).filter(Thought.used == True).all()
    for s in tmp_thoughts:
        ts = set_gt_node(s, tmp_situations, "false")
        if ts !=0:
            thoughts.append( ts )
            thoughts_new_nodes.append( set_new_emty_gt_node(s, "true") )  # Prepare new empty node in case a user wants to add newe gts of his own
            gts_arr.append( thoughts )
            gts_arr.append( thoughts_new_nodes )
            
    emotions = []
    emotions_new_nodes = []
    tmp_emotions = Emotion.query.filter(Emotion.hide == False).filter(Emotion.used == True).all()
    for s in tmp_emotions:
        ts = set_gt_node(s, tmp_thoughts, "false")
        if ts != 0:
            emotions.append( ts )
            emotions_new_nodes.append( set_new_emty_gt_node(s, "true") )
            gts_arr.append( emotions )
            gts_arr.append( emotions_new_nodes )
        
    behaviors = []
    behaviors_new_nodes = []
    tmp_behaviors = Behavior.query.filter(Behavior.hide == False).filter(Behavior.used == True).all()        
    for s in tmp_behaviors:
        ts = set_gt_node(s, tmp_emotions, "false")
        if ts !=0:
            behaviors.append( ts )
            behaviors_new_nodes.append( set_new_emty_gt_node(s, "true") )  # Prepare new empty node in case a user wants to add newe gts of his own
            gts_arr.append( behaviors )
            gts_arr.append( behaviors_new_nodes )
            
    results = []
    results_new_nodes = []
    tmp_results = Result.query.filter(Result.hide == False).filter(Result.used == True).all()
    ...
    print("GTS BBBBBBBBBBB behaviors: ", behaviors)
    print("")
    print("GTS EEEEEEEEEEEEEEE emotions: ", emotions)
    print("")
...
@login_required
def set_gt_node(gt, parent_arr, user_node):

    print("IN set_gt_node Doing parent_arr  : gt.class_name ", parent_arr, gt.class_name) 
    print("")
    print("")
    ...
    if gt.class_name!='Situation' and parent_id==0 and parent_arr!=0:
        print("IN set_gt_node Doing nothing and returning 0 : parent_arr, gt.class_name ", parent_arr, gt.class_name, parent_id)
        print("")
        return 0
                
    print("DDDD OING ************************ gt.class_name ", gt.class_name, parent_id, )
    print("")
...
@std.route('/set_new_emty_gt_node', methods=['GET', 'POST'])
@login_required
def set_new_emty_gt_node(gt_to_copy, user_node):

    user_gts_len = eval(gt_to_copy.class_name).query.count()
    
    if user_gts_len > 5:
        print("Enough gts of {0} , return empty arr ".format(gt_to_copy.class_name) ) 
        return []
        
    new_gt = eval(gt_to_copy.class_name)("Enter your title", "Enter your description", get_author_id() )	
    ...
    new_gt.in_user_path = "false",    
                            
    db.session.add(new_gt)    
    db.session.commit()       
    return set_gt_node(new_gt, 0, user_node)   
